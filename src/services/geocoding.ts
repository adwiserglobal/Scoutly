...
const BUSINESS_TERMS = [
  ...
  'salao', 'salão', 'autoescola', 'escola', 'supermercado', 'mercado', 'loja',
];
...
function looksLikeBusinessSearch(query: string): boolean {
  ...
  if (/\s+(em|no|na|perto de|perto do|perto da)\s+/.test(normalized)) {
    return true;
  }
  return BUSINESS_TERMS.some((term) => normalized.includes(normalizeText(term)));
}
...
async function searchBusinesses(query: string): Promise<GeocodedLocation | null> {
  try {
...
export async function searchAddressOrCity(query: string): Promise<GeocodedLocation | null> {
  ...
  if (looksLikeBusinessSearch(trimmed)) {
    const businessResult = await searchBusinesses(trimmed);
