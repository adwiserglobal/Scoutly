...
const BUSINESS_TERMS = [
  ...
  'salao', 'salão', 'autoescola', 'supermercado', 'mercado', 'loja', 'restaurante',
];
...
async function searchBusinesses(query: string, currentRegionName?: string): Promise<GeocodedLocation | null> {
  try {
    ...
    // Keep even an empty targeted result active so the map does not immediately replace
    // "0 results" with unrelated generic businesses from the same viewport.
