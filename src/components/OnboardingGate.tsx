...
export default function OnboardingGate() {
  ...
  const submitAnswers = async (answers: OnboardingAnswers) => {
    ...
    // New accounts are activated on Free server-side; continue directly into the product tutorial.
    setMode('tutorial');
  };
  ...
  const finishTutorial = async () => {
    try {
      ...
      setMode('hidden');
      window.dispatchEvent(new CustomEvent('scoutly-entitlements-changed'));
    }
  };
